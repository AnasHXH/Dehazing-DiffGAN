...
#     def gaussian_pyramid(self, x, levels):
#         gaussian_pyr = [x]
#         for i in range(levels):
#             x = F.avg_pool2d(x, kernel_size=2, stride=2)
#             gaussian_pyr.append(x)
#         return gaussian_pyr

#     def forward(self, x):
#         gaussian_pyr = self.gaussian_pyramid(x, self.num_levels)
#         laplacian_pyr = []

#         for i in range(self.num_levels):
#             current_gaussian = gaussian_pyr[i]
#             upsampled = F.interpolate(gaussian_pyr[i + 1], scale_factor=2, mode='bilinear', align_corners=False)
#             if upsampled.size() != current_gaussian.size():
#                 # Correct the size due to the off-by-one sizes after upscaling
#                 upsampled = F.interpolate(upsampled, size=current_gaussian.size()[2:], mode='bilinear', align_corners=False)
#             laplacian = current_gaussian - upsampled
#             laplacian_pyr.append(laplacian)

#         return laplacian_pyr[0]
...
class Get_gradient_nopadding(nn.Module):
    def __init__(self):
        ...
        kernel = torch.matmul(k.t(), k).unsqueeze(0).repeat(3, 1, 1, 1)  # Repeating the kernel for 3 channels
        self.register_buffer('kernel', kernel)  # Using register_buffer to keep it as a constant weight
    ...
    def forward(self, img):
        ...
        filtered = self.conv_gauss(img, self.kernel)  # Gaussian filter
        down = filtered[:, :, ::2, ::2]  # Downsample
        new_filter = torch.zeros_like(filtered)
        new_filter[:, :, ::2, ::2] = down * 4  # Upsample
        filtered = self.conv_gauss(new_filter, self.kernel)  # Gaussian filter again
        diff = img - filtered
        return diff
####################
# Generator
